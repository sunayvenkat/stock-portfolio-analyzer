...
def calculate_return_contribution(portfolio_values, stock_returns):
    contributions = {}
    ...
    for ticker, data in portfolio_values.items():
        ...
        if ticker not in stock_returns:
            raise ValueError(f"Missing return for ticker: {ticker}")

        contribution = (weight * stock_returns[ticker])
...
def calculate_stock_cumulative_returns(historical_prices):
    returns = {}
...
def build_attribution_table(portfolio_values, stock_returns):
    rows = []

    contributions = calculate_return_contribution(portfolio_values, stock_returns)
